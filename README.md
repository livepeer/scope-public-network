...
`serviceAddr` on `https://${DOMAIN}`. Live runners register themselves at
startup, so the default config does not set `liveRunnerAddr`; if your
go-livepeer build supports that option and you need a custom callback URL, add
it intentionally after verifying the image accepts the key. The runner itself
is still reached internally at `http://scope-live-runner:8989`. Scope stores
persistent shared runner data at `/workspace/shared` and session-specific data
under `/tmp/.daydream-scope/assets`.
